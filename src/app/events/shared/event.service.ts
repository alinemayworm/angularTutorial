...
export class EventService {
  ...
  searchSessions(searchTerm: string) {
    ...
    /* EVENTS.forEach(event => {
      ...
      matchingSessions = matchingSessins.map((session: any) => {
    ...
    for (let i = 0; i < EVENTS.length; i++) {
      for (let j = 0; j < EVENTS[i].sessions.length; j++) {
       if (EVENTS[i].sessions[j].name.toLocaleLowerCase().includes(term)) {
          EVENTS[i].sessions[j].eventId = EVENTS[i].id;
         results.push(EVENTS[i].sessions[j]);
      }
      }
    }
    ...
    sessions: [
      {
        ...
        duration: 1,
        level: 'Intermediate',
        abstract: `Learn all abo
          ut the new pipes in Angular 4, both
